...
def isolate_module(mod):
    ...
    If a tests suite mocks os.path.exists (for example), and then we need to use
    it during the tests, everything will get tangled up if we use their mock.
...
# we are running our own tests suite.
if USE_CONTRACTS:
...
class StopEverything(BaseCoverageException):
    ...
    tests, raising this exception will automatically skip the tests.
