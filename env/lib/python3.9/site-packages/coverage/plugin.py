...
* Dynamic context switchers decide when the dynamic context has changed, for
  example, to record what tests function produced the coverage.
...
name starts with 'tests'.  Such a plugin would provide basic coverage grouping
by tests and could be used with tests runners that have no built-in coveragepy
support.
