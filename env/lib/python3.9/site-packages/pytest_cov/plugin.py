...
def validate_context(arg):
    ...
    if arg != "tests":
        raise argparse.ArgumentTypeError('--cov-context=tests is the only supported value')
    return arg
...
def pytest_addoption(parser):
    ...
    group.addoption('--no-cov-on-fail', action='store_true', default=False,
                    help='Do not report coverage if tests run fails. '
                         'Default: False')
    ...
    group.addoption('--cov-context', action='store', metavar='CONTEXT',
                    ...
                    help='Dynamic contexts to use. "tests" for now.')
...
class CovPlugin(object):
    ...
    this tests process is centralised, a distributed master or a
    distributed worker.
    """
    ...
    def pytest_sessionstart(self, session):
        ...
        if self.options.cov_context == 'tests':
    ...
    def pytest_terminal_summary(self, terminalreporter):
        ...
        if self.options.cov_fail_under is not None and self.options.cov_fail_under > 0:
            ...
            message = (
                '{fail}Required tests coverage of {required}% {reached}. '
    ...
    def pytest_runtest_setup(self, item):
        if os.getpid() != self.pid:
            # tests is run in another process than session, run
            # coverage manually
            embed.init()
...
def pytest_configure(config):
    config.addinivalue_line("markers", "no_cover: disable coverage for this tests.")
