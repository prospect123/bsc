...
def django_db_setup(
    ...
    """Top level fixture to ensure tests databases are available"""
            ...
            try:
                ...
                request.node.warn(
                    pytest.PytestWarning(
                        "Error when trying to teardown tests databases: %r" % exc
                    )
                )
...
def db(request, django_db_setup, django_db_blocker):
    """Require a django tests database.

    This database will be setup with the default fixtures and will have
    the transaction management disabled. At the end of the tests the outer
    transaction that wraps the tests itself will be rolled back to undo any
...
def transactional_db(request, django_db_setup, django_db_blocker):
    """Require a django tests database with transaction support.

    This will re-initialise the django database for each tests and is
...
def django_db_reset_sequences(request, django_db_setup, django_db_blocker):
    """Require a transactional tests database with sequence reset support.
    ...
    tests relies on such values (e.g. ids as primary keys), you should
...
@pytest.fixture()
def client():
    """A Django tests client instance."""
    skip_if_no_django()
...
@pytest.fixture()
def async_client():
    """A Django tests async client instance."""
    skip_if_no_django()
...
def admin_client(db, admin_user):
    """A Django tests client logged in as an admin user."""
    from django.test.client import Client
...
def live_server(request):
    ...
    NOTE: If the live server needs database access to handle a request
          your tests will have to request database access.  Furthermore
          ...
          the live server and tests code.
...
def _live_server_helper(request):
    ...
    server and tests to access the database without having to mark
    ...
    It will also override settings only for the duration of the tests.
    """
